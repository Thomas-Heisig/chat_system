...
class Message(BaseDatabaseModel):
    ...
    username: str = Field(
        ..., min_length=1, max_length=100, description="User who sent the message"
    )
    message: str = Field(..., min_length=1, max_length=10000, description="Message content")
    message_compressed: Optional[bytes] = Field(
        default=None, description="Compressed message content"
    )
    ...
    # AI-related fields
    is_ai_response: bool = Field(
        default=False, description="Whether this is an AI-generated response"
    )
    ai_model_used: Optional[str] = Field(default=None, description="AI model used for generation")
    context_message_ids: List[int] = Field(
        default_factory=list, description="IDs of context messages"
    )
    rag_sources: List[Dict[str, Any]] = Field(
        default_factory=list, description="RAG source documents"
    )
...
class User(BaseDatabaseModel):
    ...
    force_password_change: bool = Field(
        default=False, description="Whether user must change password on next login"
    )
...
class Project(BaseDatabaseModel):
    ...
    progress_percentage: float = Field(
        default=0.0, ge=0.0, le=100.0, description="Project progress"
    )
...
class RoomMember(BaseDatabaseModel):
    ...
    custom_settings: Dict[str, Any] = Field(
        default_factory=dict, description="Custom member settings"
    )
...
class AIConversation(BaseDatabaseModel):
    ...
    conversation_settings: Dict[str, Any] = Field(
        default_factory=dict, description="Conversation settings"
    )
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
