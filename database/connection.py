...
def _verify_database_setup():
    ...
    required_tables = [
        'messages', 'users', 'projects', 'tickets', 'files', 
        'message_reactions', 'chat_rooms', 'room_members',
        'ai_conversations', 'ai_models', 'api_credentials', 'audit_log'
    ]
    ...
    for model in default_models:
        conn.execute(
            """INSERT INTO ai_models (id, name, model_type, provider, config, capabilities) 
               VALUES (?, ?, ?, ?, ?, ?)""",
            (model['id'], model['name'], model['model_type'], model['provider'], 
...
def _insert_default_admin_user(conn):
    ...
    conn.execute(
        """INSERT INTO users (id, username, email, password_hash, display_name, role, is_verified)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (admin_id, 'admin', 'admin@localhost', password_hash, 'Administrator', 'admin', True)
    )
    
    logger.warning("⚠️ Default admin user created - PLEASE CHANGE PASSWORD IMMEDIATELY")
    ...
    try:
        with get_db_connection() as conn:
            ...
            if cursor.fetchone()[0] == 1:
                health['checks']['connectivity'] = {'status': 'pass', 'message': 'Database responsive'}
        ...
        failed_checks = [check for check, details in health['checks'].items() 
                        if details['status'] == 'fail']
        warning_checks = [check for check, details in health['checks'].items() 
