...
class AuthService:
    ...
    def __init__(self):
        ...
        if PASSLIB_AVAILABLE and CryptContext is not None:
            try:
                ...
                enhanced_logger.warning("Failed to initialize passlib CryptContext, falling back", error=str(e))
        ...
        enhanced_logger.info("AuthService initialized",
    ...
    def _create_simple_token(self, user_id: str, username: str, role: str) -> str:
        ...
        token_data = {
            ...
            "username": username,
            "role": role,
            "exp": (datetime.utcnow() + timedelta(minutes=self.access_token_expire_minutes)).isoformat()
        }
    ...
    def _verify_simple_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            expected_signature = hashlib.sha256(f"{data_str}{self.secret_key}".encode()).hexdigest()[:16]
    ...
    def create_api_key(self, user_id: str, name: str, 
        ...
        enhanced_logger.info("API key created",
                           key_id=key_id,
                           user_id=user_id,
                           name=name)
    ...
    def get_user_api_keys(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for key, data in self._api_keys.items():
            if data['user_id'] == user_id:
                keys.append({
                    **data,
                    "api_key": f"{key[:8]}...{key[-4:]}"  # Masked key
                })
        return keys
