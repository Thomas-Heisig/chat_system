...
from .base_rag import BaseRAGProvider, Document, DocumentType, SearchResult
...
class QdrantRAGProvider(BaseRAGProvider):
            ...
            if self.collection_name not in collection_names:
                ...
                self._client.create_collection(
                    ...
                    optimizers_config=qdrant_models_module.OptimizersConfigDiff(
                        default_segment_number=2, max_segment_size=50000, memmap_threshold=20000
                    ),
                    # Enable persistence
                    hnsw_config=qdrant_models_module.HnswConfigDiff(m=16, ef_construct=100),
                )
    ...
    async def add_documents(self, documents: List[Document]) -> bool:
        ...
        try:
            ...
            for doc in documents:
                ...
                points.append(
                    qdrant_models_module.PointStruct(
                ...
                for key, value in filters.items():
                    conditions.append(
                        qdrant_models_module.FieldCondition(
                            key=key, match=qdrant_models_module.MatchValue(value=value)
                        )
                    )
    ...
    async def update_document(self, doc_id: str, document: Document) -> bool:
        ...
        try:
            ...
            self._client.upsert(
                ...
                points=[
                    qdrant_models_module.PointStruct(
                        id=point_id, vector=document.embedding, payload=payload
                    )
                ],
                wait=True,
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            count_result = self._client.count(
                collection_name=self.collection_name, exact=False  # Faster approximate count
            )
    ...
    async def clear_collection(self) -> bool:
        ...
        try:
            ...
            self._client.create_collection(
                collection_name=self.collection_name,
                vectors_config=qdrant_models_module.VectorParams(
                    size=self._embedding_dimension, distance=qdrant_models_module.Distance.COSINE
                ),
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            count_result = self._client.count(collection_name=self.collection_name, exact=False)
    ...
    async def create_snapshot(self, snapshot_path: str) -> bool:
        ...
        try:
            ...
            snapshot_info = self._client.create_snapshot(collection_name=self.collection_name)
