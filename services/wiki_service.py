...
class WikiService:
    ...
    async def update_page(
        self, page_id: str, content: str, editor: str, comment: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def restore_version(self, page_id: str, version: int, editor: str) -> Dict[str, Any]:
        ...
        for version_entry in self.page_versions[page_id]:
            if version_entry["version"] == version:
                return await self.update_page(
                    page_id, version_entry["content"], editor, f"Restored from version {version}"
                )
    ...
    async def get_recent_pages(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_pages = sorted(
            self.pages.values(), key=lambda x: x.get("updated_at", ""), reverse=True
        )
