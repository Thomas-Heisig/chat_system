...
class AIProvider:
    ...
    def __init__(self, name: str, config: Dict[str, Any], default_model: Optional[str] = None):
        self.name = name
        self.config = config
...
class OllamaProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        super().__init__("ollama", config, config.get("default_model", "llama2"))
        self.base_url: str = config.get("base_url", "http://localhost:11434")
        self.default_model: str = config.get("default_model", "llama2")
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    enhanced_logger.error(
                        "Ollama API error", extra={"status": response.status_code}
                    )
                    return ""
        except Exception as e:
...
class AIService:
    ...
    async def initialize(self):
        ...
        if await ollama.check_availability():
            ...
            enhanced_logger.info(
                "Ollama provider initialized", extra={"models": await ollama.get_models()}
            )

        self._initialized = True
        enhanced_logger.info(
            "AI Service initialized",
            extra={"providers": list(self.providers.keys()), "default": self.default_provider},
        )
    ...
    async def generate_response(
        ...
        if not provider_name or provider_name not in self.providers:
            return {"success": False, "error": "No AI provider available", "response": None}
        ...
        try:
            ...
            enhanced_logger.error(
                "AI generation failed", provider=provider_name, error=str(e), duration=duration
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for name, provider in self.providers.items():
            ...
            provider_health[name] = {
                "available": available,
                "config": {
                    k: v for k, v in provider.config.items() if k not in ["api_key", "password"]
                },
            }
