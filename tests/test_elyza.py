...
import pytest
from services.elyza_service import ElyzaService, get_elyza_service, Language, SentimentType
...
async def test_generate_response_with_user_context(elyza_service):
    ...
    # First message
    result1 = await elyza_service.generate_response(
        "Hallo",
        user_id=user_id
    )
    
    # Second message (should have context)
    result2 = await elyza_service.generate_response(
        "Wie heißt du?",
        user_id=user_id
    )
