...
async def test_get_page(wiki_service):
    ...
    # Create a page
    created = await wiki_service.create_page(title="Get Test", content="Content", author="user")
...
async def test_get_page_by_title(wiki_service):
    ...
    await wiki_service.create_page(title="Unique Title", content="Content", author="user")
...
async def test_update_page(wiki_service):
    ...
    created = await wiki_service.create_page(
        title="Update Test", content="Original content", author="user"
    )
    ...
    result = await wiki_service.update_page(
        page_id=page_id, content="Updated content", editor="editor", comment="Updated for testing"
    )
...
async def test_page_history(wiki_service):
    ...
    created = await wiki_service.create_page(
        title="History Test", content="Version 1", author="user"
    )
...
async def test_search_pages(wiki_service):
    ...
    await wiki_service.create_page(
        "Docker Guide", "Docker is a container platform", "user", category="devops"
    )
    await wiki_service.create_page(
        "Kubernetes Guide", "Kubernetes orchestrates containers", "user", category="devops"
    )
...
async def test_delete_page(wiki_service):
    ...
    # Create page
    created = await wiki_service.create_page("Delete Test", "To be deleted", "user")
