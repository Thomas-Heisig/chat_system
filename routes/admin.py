...
from database.connection import check_database_health, get_database_stats
...
async def list_api_keys():
    ...
    try:
        ...
        return {"api_keys": [], "total": 0}  # Would be populated from auth_service
...
async def create_api_key(
    ...
    try:
        ...
        enhanced_logger.info("API key created by admin", key_id=result["id"], user_id=user_id)
...
async def cleanup_sessions(
    ...
    try:
        ...
        return {"status": "completed", "removed_sessions": removed, "max_age_hours": max_age_hours}
...
async def get_system_health():
    ...
    try:
        ...
        unhealthy = any(c.get("status") == "unhealthy" for c in health["components"].values())
        if unhealthy:
