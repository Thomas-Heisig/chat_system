from fastapi import APIRouter, WebSocket, HTTPException, Depends, Query, UploadFile, File, Form, BackgroundTasks
...
from database.repositories import (
    ...
    TicketRepository, FileRepository, SearchRepository, StatisticsRepository
)
...
try:
    ...
    logger.info(f"🚀 System Features - Projects: {settings.FEATURE_PROJECT_MANAGEMENT}, "
...
async def get_chat_page():
    ...
    try:
        ...
        logger.info(f"🌐 Enhanced chat page served - "
...
async def get_ai_models():
    ...
    try:
        ...
        logger.info(f"📋 AI models requested - Found: {len(models['ollama'])} Ollama, "
...
async def ask_ai_question(
    ...
    try:
        ...
        logger.info(f"✅ AI response generated - Sources: {len(result.get('sources', []))}, "
...
async def upload_file(
    ...
    try:
        ...
        background_tasks.add_task(
            file_service.analyze_uploaded_file,
            file_record.id
        )
...
async def get_filtered_messages(
    ...
    try:
        ...
        logger.info(f"✅ Filtered messages - Returned: {len(result.items)}, "
