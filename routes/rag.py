...
from services.rag import DocumentProcessor, get_available_providers, get_rag_provider
    ...
    try:
        ...
        if success:
            enhanced_logger.info(
                "Document added to RAG", chunk_count=len(chunks), doc_type=doc_type
            )
            return {
                "status": "added",
...
async def upload_document(file: UploadFile = File(...), metadata: str = Form("{}")):
    ...
    try:
        ...
        try:
            ...
            if success:
                enhanced_logger.info(
                    "Document file uploaded and indexed",
...
async def search_documents(
    ...
    try:
        ...
        enhanced_logger.info("RAG search performed", query=query[:50], results_count=len(results))
...
async def get_rag_stats():
    ...
    try:
        ...
        return {"stats": stats, "health": health, "timestamp": datetime.now().isoformat()}
    ...
    try:
        ...
        for i, result in enumerate(results, 1):
            context_parts.append(
                f"[{i}] (Score: {result.score:.2f}) {result.document.content[:200]}..."
            )
