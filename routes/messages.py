...
from database.models import Message, MessageFilter, MessageType, PaginatedResponse
...
async def get_all_messages(
    ...
    try:
        ...
        duration = (datetime.now() - start_time).total_seconds()
        enhanced_logger.error("Failed to retrieve messages", error=str(e), duration=duration)
...
async def get_recent_messages(
    ...
    try:
        logger.info(
            f"🕒 GET /messages/recent - Limit: {limit}, Room: {room_id}, Project: {project_id}"
        )
        ...
        logger.info(
            f"✅ Returned {len(messages)} recent messages - "
...
async def get_user_messages(
    ...
    try:
        filters = MessageFilter(
            username=username, project_id=project_id, limit=limit, offset=offset
        )

        logger.info(
            f"👤 GET /messages/user/{username} - "
        ...
        logger.info(
            f"✅ Returned {len(result.items)} messages from user '{username}' "
...
async def get_message_context(
    ...
    try:
        ...
        logger.info(
            f"✅ Context retrieved for message {message_id} - "
...
async def add_message_reaction(
    ...
    try:
        logger.info(
            f"👍 POST /messages/{message_id}/reactions - " f"User: {user_id}, Reaction: {reaction}"
        )
...
async def get_message_stats(
    ...
    try:
        ...
        if timeframe == "today":
            ...
            all_messages = [
                msg for msg in all_messages if msg.timestamp and msg.timestamp >= cutoff
            ]
        elif timeframe == "week":
        ...
        if all_messages and all_messages[0].timestamp:
            messages_sorted = sorted(
                all_messages, key=lambda x: x.timestamp if x.timestamp is not None else datetime.min
            )
        ...
        stats = {
            ...
            "ai_responses": {
                "total": ai_responses,
                "percentage": (
                    round((ai_responses / total_messages) * 100, 1) if total_messages > 0 else 0
                ),
        ...
        logger.info(
            f"📊 Comprehensive statistics generated - "
...
async def get_message_count(
    ...
    try:
        filters = MessageFilter(
            project_id=project_id, room_id=room_id, limit=1, offset=0  # We only need count
        )
        ...
        logger.info(
            f"🔢 GET /messages/count - Total: {result.total}, "
...
async def search_messages(
    ...
    try:
        ...
        logger.info(
            f"🔍 GET /messages/search - Query: '{query}', Type: {search_type}, Limit: {limit}"
        )
...
async def get_project_messages(
    ...
    try:
        ...
        filters = MessageFilter(project_id=project_id, limit=limit, offset=offset)
...
async def export_messages(
    ...
    try:
        logger.info(
            f"📤 GET /messages/export - Format: {format}, " f"Start: {start_date}, End: {end_date}"
        )
        ...
        logger.info(
            f"✅ Messages export prepared - Format: {format}, " f"Messages: {len(result.items)}"
        )
