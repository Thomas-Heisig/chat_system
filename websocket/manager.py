...
class ConnectionManager:
    ...
    async def send_personal_message(self, message: dict, websocket: WebSocket, 
        ...
        try:
            if websocket not in self.active_connections:
                enhanced_logger.warning(
                    "Attempted to send message to disconnected client",
    ...
    async def leave_room(self, websocket: WebSocket, room_id: str) -> bool:
        ...
        try:
            if (room_id in self.room_connections and 
                websocket in self.room_connections[room_id]):
    ...
    def get_room_stats(self, room_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'users': list(users),
            'active_since': min(
                [self.connection_info[conn]['connected_at'] for conn in connections 
    ...
    async def cleanup_inactive_connections(self, max_inactive_seconds: int = 300):
        ...
        if inactive_connections:
            enhanced_logger.info(
                "Inactive connections cleaned up",
