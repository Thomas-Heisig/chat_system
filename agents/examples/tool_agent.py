...
class ToolAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "tool_agent_001"):
        super().__init__(
            agent_id=agent_id,
            name="Tool Agent",
            capabilities=[AgentCapability.TOOL_USE, AgentCapability.ANALYSIS],
    ...
    def register_tool(
        ...
        self.tool_metadata[name] = {"description": description, "parameters": parameters}
    ...
    async def _execute_tool(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tool_name not in self.tools:
            return {"error": f"Tool not found: {tool_name}", "agent_id": self.agent_id}

        try:
            tool_function = self.tools[tool_name]
            result = (
                await tool_function(**parameters)
                if asyncio.iscoroutinefunction(tool_function)
                else tool_function(**parameters)
            )
    ...
    async def _list_tools(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tools_list = [{"name": name, **metadata} for name, metadata in self.tool_metadata.items()]

        return {"tools": tools_list, "count": len(tools_list), "agent_id": self.agent_id}
    ...
    async def _get_tool_info(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"name": tool_name, **self.tool_metadata[tool_name], "agent_id": self.agent_id}
