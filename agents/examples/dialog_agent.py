...
class DialogAgent(BaseAgent):
    ...
    async def _analyze_intent(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        intents = {
            "question": any(word in user_input.lower() for word in ["what", "how", "why", "when", "where", "who", "?"]),
            "command": any(word in user_input.lower() for word in ["do", "make", "create", "delete", "update"]),
            "greeting": any(word in user_input.lower() for word in ["hello", "hi", "hey", "greetings"]),
